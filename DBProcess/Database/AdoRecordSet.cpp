...
void CAdoRecordSet::Release()
{
	...
	{
		TRACE(_T("Warning: Release关闭记录集发生异常. 错误信息: %s; 文件: %s; 行: %d\n"), e.ErrorMessage(), __FILE__, __LINE__);
	}
}
...
BOOL CAdoRecordSet::GetCollect(LPCTSTR strFieldName, char *chValue, int iInSize)
{
	CString strValue;

	if (!GetCollect(strFieldName, strValue))
	{
		return FALSE;
	}

	sprintf_s(chValue, iInSize, "%s", strValue);
	return TRUE;
}
...
void CAdoRecordSet::getError(const _com_error& e)
{
//	LOG(LEVEL_ERROR, "Get Error:%s Description:%s", e.ErrorMessage(), e.Description());
	throw e;
}

void CAdoRecordSet::getError()
{
//	LOG(LEVEL_ERROR, "get Error:数据字段类型不匹配");
	throw 1;
}
